...
const DDRTimingCalculator = () => {
  ...
  const [mode, setMode] = useState('standard'); // 'standard' | 'wizard'
      ...
      <div className="zdiff-header">
        ...
        <div className="flex gap-2">
          ...
          <div className="zdiff-toggle-group">
            <button
              className={`zdiff-toggle-btn ${mode === 'standard' ? 'zdiff-toggle-btn--active-orange' : ''}`}
              onClick={() => setMode('standard')}
            >
              Standard
            </button>
            <button
              className={`zdiff-toggle-btn ${mode === 'wizard' ? 'zdiff-toggle-btn--active-orange' : ''}`}
        ...
        {mode === 'standard' ? (
          ...
          <div className="w-full p-4 bg-white/5 rounded-lg border border-white/10">
            ...
            <div className="flex justify-between items-center mb-6">
              <div>
                <span className="text-xs font-bold uppercase text-tertiary">Step {currentStep} of 4</span>
                <h4 className="text-sm font-bold text-white mt-1">
                  {currentStep === 1 && "Technology & Speed"}
                  {currentStep === 2 && "Board Material"}
                  {currentStep === 3 && "Layout Measurement"}
                  {currentStep === 4 && "Results & Verdict"}
                </h4>
              </div>
              <div className="flex gap-1">
                {[1, 2, 3, 4].map(s => (
                  <div key={s} className={`h-1.5 w-6 rounded-full transition-all ${s <= currentStep ? 'bg-orange-500' : 'bg-white/10'}`} />
                ))}
              </div>
            </div>
            ...
            <div className="mb-8 min-h-[120px] flex flex-col justify-center">
              {currentStep === 1 && (
                <div className="slide-up">
                  <p className="text-xs text-tertiary mb-3">Select the DDR generation and the data rate for your design.</p>
                  <div className="zdiff-input-group mb-4">
                    <div className="zdiff-toggle-group w-full">
                      {Object.keys(generations).map(gen => (
                        <button 
                          key={gen} 
              ...
              {currentStep === 2 && (
                <div className="slide-up">
                  <p className="text-xs text-tertiary mb-3">Enter the dielectric constant (Er) of your PCB material.</p>
                  <EngineeringInput
                    label="εr (Dielectric Constant)"
                    unit="Dk"
                    value={er}
              ...
              {currentStep === 3 && (
                <div className="slide-up">
                  <p className="text-xs text-tertiary mb-3">Enter the maximum length mismatch (skew) between your traces.</p>
                  <EngineeringInput
                    label="Total PCB Row Skew"
                    unit={unitSystem}
                    value={convertValue(skew)}
                ...
                <div className="slide-up">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="zdiff-result-card" style={{ borderColor: stats.statusColor + '44' }}>
                      ...
                      <div className="zdiff-result-sub-grid mt-2">
                        <div className="zdiff-result-sub">
                          <div className="zdiff-result-sub-label">Bit Time</div>
                          <div className="zdiff-result-sub-val">{stats.ui.toFixed(1)} <small>ps</small></div>
                        </div>
                        <div className="zdiff-result-sub">
                          <div className="zdiff-result-sub-label">Skew Delay</div>
